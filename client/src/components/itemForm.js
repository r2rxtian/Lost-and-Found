...
export function openItemForm(type, meta, existing, onSaved) {
  ...
  const body = `<form id="item-form"><div class="form-grid">
    ...
    <div class="field full field-description"><label>Description *</label><textarea name="description" required>${escapeHtml(item.description)}</textarea></div>
    <div class="field full field-private"><label>Private Verification Details</label><textarea name="privateDetails">${escapeHtml(item.privateDetails)}</textarea><small>Enter identifying details that should NOT be shown publicly, such as exact contents, serial numbers, unique markings, or damage.</small></div>
    ...
    <div class="field field-upload ${type === 'found' ? 'with-custody' : 'full'}"><label>${existing ? 'Replace Photo' : type === 'found' ? 'Upload Photo' : 'Optional Photo'}</label><label class="upload-zone" id="upload-zone"><input hidden type="file" name="image" accept="image/jpeg,image/png,image/webp"><div id="upload-copy"><i data-lucide="image-up"></i><p>Drop an image here or click to browse</p><small>JPG, PNG, or WEBP · up to 5 MB</small></div></label></div>
  ...
  const close = openModal(`${existing ? 'Edit' : 'Report'} ${word} Item`, body, { wide: true, fixed: true });
